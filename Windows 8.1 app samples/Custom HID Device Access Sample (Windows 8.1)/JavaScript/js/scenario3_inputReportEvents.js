...
(function () {
    ...
    var page = WinJS.UI.Pages.define("/html/scenario3_inputReportEvents.html", {
        ...
        unload: function () {
            ...
            if (SdkSample.CustomHidDeviceAccess.eventHandlerForDevice.current.isDeviceConnected) {
                // Unregister events
                if (inputReportEvents.isRegisteredForInputReportEvents) {
                ...
                if (SdkSample.CustomHidDeviceAccess.utilities.isSuperMuttDevice(SdkSample.CustomHidDeviceAccess.eventHandlerForDevice.current.device)) {
    ...
    function registerForInputReportEventsClick(eventArgs) {
        if (SdkSample.CustomHidDeviceAccess.eventHandlerForDevice.current.isDeviceConnected) {
            ...
            // This call is made here instead of ready() because if the device is disconnected and reconnected, we need to re-enable interrupts
            if (SdkSample.CustomHidDeviceAccess.utilities.isSuperMuttDevice(SdkSample.CustomHidDeviceAccess.eventHandlerForDevice.current.device)) {
