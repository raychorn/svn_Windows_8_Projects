...
Platform::String^ DeviceConnect::ButtonNameDisableReconnectToDevice = "Do not automatically reconnect to device that was just removed";
...
void DeviceConnect::ConnectToDevice_Click(Object^ /* sender */, RoutedEventArgs^ /* eventArgs */)
{
    ...
    if (selection->Size > 0)
    {
        ...
        entry = dynamic_cast<DeviceListEntry^>(obj);

        if (entry != nullptr)
        {
            // Since we're connecting to a new device, we need create a new event handler for this device
            EventHandlerForDevice::CreateNewEventHandlerForDevice();

            MainPage::Current->Dispatcher->RunAsync(
                CoreDispatcherPriority::Normal,
                ref new DispatchedHandler([this, entry] () -> void 
                {
...
void DeviceConnect::OnDeviceEnumerationComplete(DeviceWatcher^ /* sender */, Object^ /* args */)
{
        ...
        ref new DispatchedHandler([this]()
    {
        if (EventHandlerForDevice::Current->IsDeviceConnected)
        {
            MainPage::Current->NotifyUser("Currently connected to: " + EventHandlerForDevice::Current->DeviceInformation->Id,
