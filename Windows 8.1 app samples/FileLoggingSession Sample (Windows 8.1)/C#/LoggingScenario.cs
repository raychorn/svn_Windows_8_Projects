...
namespace FileLoggingSession
{
    ...
    class LoggingScenario
    {
        ...
        private async void LogFileGeneratedHandler(IFileLoggingSession sender, LogFileGeneratedEventArgs args)
        {
            ...
            string finalLogFileFullPath = await MoveFileToRepositoryAsync(args.File);
            if (IsSuspending == false)
            {
                if (StatusChanged != null)
                {
                    StatusChanged.Invoke(this, new LoggingScenarioEventArgs(LoggingScenarioEventType.LogFileGenerated, finalLogFileFullPath));
                }
            }
        }
        ...
        public async Task<bool> ToggleLoggingEnabledDisabledAsync()
        {
            ...
            {
                bool enabled;
                Windows.Foundation.Diagnostics.FileLoggingSession sessionCopy = Interlocked.Exchange(ref session, null);
                if (sessionCopy != null)
                {
                    string finalLogFilePath = await SaveFinalLogFileAsync(sessionCopy);
                    sessionCopy.Dispose();
                    if (StatusChanged != null)
                    {
        ...
        private async Task<string> SaveFinalLogFileAsync(Windows.Foundation.Diagnostics.FileLoggingSession sessionToCloseAndSave)
        {
            StorageFile finalFileBeforeSuspend = await sessionToCloseAndSave.CloseAndSaveToFileAsync();
            if (finalFileBeforeSuspend != null)
            {
                string finalAppLogFileFullPath = await MoveFileToRepositoryAsync(finalFileBeforeSuspend);
        ...
        private async Task<string> MoveFileToRepositoryAsync(StorageFile sourceFile)
        {
            StorageFolder logRepositoryFolder =
                await ApplicationData.Current.LocalFolder.CreateFolderAsync(LOG_FILE_REPOSITORY_FOLDER_NAME,
                                                                            CreationCollisionOption.OpenIfExists);
            await moveLogSemaphore.WaitAsync();
            try
            {
                string fileNameToUse = await GetNextLogFileNameAsync();
                await sourceFile.MoveAsync(logRepositoryFolder, fileNameToUse, NameCollisionOption.FailIfExists);
                string fullFilePath = System.IO.Path.Combine(logRepositoryFolder.Path, fileNameToUse);
                return fullFilePath;
            }
            finally
            {
                moveLogSemaphore.Release();
            }
        }
        ...
        public async Task PrepareToSuspendAsync()
        {
            ...
            if (sessionCopy != null)
            {
                ...
                try
                {
                    string finalFileBeforeSuspend = await SaveFinalLogFileAsync(sessionCopy);
                    sessionCopy.Dispose();
                    ApplicationData.Current.LocalSettings.Values["LoggingEnabled"] = true;
