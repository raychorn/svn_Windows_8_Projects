...
(function () {
    ...
    var InterruptPipesClass = WinJS.Class.define(null, {
        ...
        registerForInterruptEvent: function (pipeIndex, eventHandler) {
            if (!this.registeredInterrupt) {
                ...
                this._registeredDevice = SdkSample.CustomUsbDeviceAccess.eventHandlerForDevice.current.device;

                // Search for the correct pipe that has the specified endpoint number
                var interruptInPipe = this._registeredDevice.defaultInterface.interruptInPipes.getAt(pipeIndex);
        ...
        unregisterForInterruptEvent: function () {
            if (this.registeredInterrupt) {
                ...
                if (this._registeredDevice === SdkSample.CustomUsbDeviceAccess.eventHandlerForDevice.current.device) {
                    // Search for the correct pipe that we know we used to register events
                    var interruptInPipe = this._registeredDevice.defaultInterface.interruptInPipes.getAt(this._registeredInterruptPipeIndex);
        ...
        writeToInterruptOut: function (pipeIndex, bytesToWrite) {
            ...
            var pipe = SdkSample.CustomUsbDeviceAccess.eventHandlerForDevice.current.device.defaultInterface.interruptOutPipes.getAt(pipeIndex);
    ...
    var page = WinJS.UI.Pages.define("/html/scenario3_interruptPipes.html", {
        ...
        unload: function () {
            ...
            // Unregister events
            if (SdkSample.CustomUsbDeviceAccess.eventHandlerForDevice.current.isDeviceConnected && interruptPipes.registeredInterrupt) {
                interruptPipes.unregisterForInterruptEvent();

                // The page is not "reloaded" when navigated away and back, so we need to make reset the UI
                updateRegisterEventButton();
    ...
    function writeSuperMuttInterruptOutClick(eventArgs) {
        if (SdkSample.CustomUsbDeviceAccess.eventHandlerForDevice.current.isDeviceConnected
            ...
            var bytesToWrite = SdkSample.CustomUsbDeviceAccess.eventHandlerForDevice.current.device.defaultInterface.interruptOutPipes.getAt(outPipeIndex).endpointDescriptor.maxPacketSize;
