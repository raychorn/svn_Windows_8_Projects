...
(function () {
    ...
    var BulkPipesClass = WinJS.Class.define(null, {
        ...
        bulkReadWriteAsync: function (bulkInPipeIndex, bulkOutPipeIndex, bytesToReadWrite, state) {
            ...
            bulkPipes.readingWritingPromise = new WinJS.Promise(function (success, error) {
                function readLoop() {
    ...
    function bulkReadClick(eventArgs) {
        if (SdkSample.CustomUsbDeviceAccess.eventHandlerForDevice.current.isDeviceConnected) {
            ...
            bulkPipes.bulkReadAsync(bulkInPipeIndex, bytesToRead).done(updateButtonStates, function (error) {
    ...
    function bulkWriteClick(eventArgs) {
        if (SdkSample.CustomUsbDeviceAccess.eventHandlerForDevice.current.isDeviceConnected) {
            ...
            bulkPipes.bulkWriteAsync(bulkOutPipeIndex, bytesToWrite).done(updateButtonStates, function (error) {
                // Promise was canceled
                if (error.name === "Canceled") {
                    updateButtonStates();
    ...
    function bulkReadWriteClick(eventArgs) {
        if (SdkSample.CustomUsbDeviceAccess.eventHandlerForDevice.current.isDeviceConnected) {
            ...
            bulkPipes.bulkReadWriteAsync(bulkInPipeIndex, bulkOutPipeIndex, bytesToWrite, bulkPipes).done(updateButtonStates, function (error) {
                // Promise was canceled
                if (error.name === "Canceled") {
                    updateButtonStates();
