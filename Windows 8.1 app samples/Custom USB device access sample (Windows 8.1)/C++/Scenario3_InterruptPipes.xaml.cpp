...
void InterruptPipes::WriteSuperMuttInterruptOut_Click(Platform::Object^ /* sender */, Windows::UI::Xaml::RoutedEventArgs^ /* eventArgs */)
{
        ...
        && (Utilities::IsSuperMuttDevice(EventHandlerForDevice::Current->Device)))
    {
        ...
        auto bytesToWrite = EventHandlerForDevice::Current->Device->DefaultInterface->
            InterruptOutPipes->GetAt(SuperMutt::Pipe::InterruptOutPipeIndex)->EndpointDescriptor->MaxPacketSize;
...
void InterruptPipes::WriteToInterruptOut(uint32 pipeIndex, uint32 bytesToWrite)
{
    ...
    auto pipe = EventHandlerForDevice::Current->Device->DefaultInterface->InterruptOutPipes->GetAt(pipeIndex);
    ...
    TypedEventHandler<UsbInterruptInPipe^, UsbInterruptInEventArgs^>^ eventHandler)
{
    if (!registeredInterrupt)
    {
        ...
        auto interruptInPipe = registeredDevice->DefaultInterface->InterruptInPipes->GetAt(pipeIndex);
...
void InterruptPipes::UnregisterFromInterruptEvent(void)
{
    if (registeredInterrupt)
    {
        ...
        if (registeredDevice == EventHandlerForDevice::Current->Device)
        {
            ...
            auto interruptInPipe = registeredDevice->DefaultInterface->InterruptInPipes->GetAt(registeredInterruptPipeIndex);
...
void InterruptPipes::CreateBooleanChartInTable(
