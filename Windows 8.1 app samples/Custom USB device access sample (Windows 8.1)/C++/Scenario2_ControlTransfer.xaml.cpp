...
task<void> ControlTransfer::SetOsrFx2SevenSegmentAsync(uint8 numericValue)
{
    ...
    UsbSetupPacket^ initSetupPacket = ref new UsbSetupPacket();
    initSetupPacket->RequestType->Recipient = UsbControlRecipient::Device;
    initSetupPacket->RequestType->ControlTransferType = UsbControlTransferType::Vendor;
    initSetupPacket->Request = OsrFx2::VendorCommand::SetSevenSegment;
    initSetupPacket->Value = 0;
    initSetupPacket->Length = bufferToSend->Length;

    return create_task(EventHandlerForDevice::Current->Device->SendControlOutTransferAsync(initSetupPacket, bufferToSend))
...
task<void> ControlTransfer::SetSuperMuttLedBlinkPatternAsync(uint8 pattern)
{
    ...
    initSetupPacket->RequestType->Recipient = UsbControlRecipient::Device;
    initSetupPacket->RequestType->ControlTransferType = UsbControlTransferType::Vendor;
    initSetupPacket->Request = SuperMutt::VendorCommand::SetLedBlinkPattern;
    initSetupPacket->Value = pattern;
    initSetupPacket->Length = 0;

    return create_task(EventHandlerForDevice::Current->Device->SendControlOutTransferAsync(initSetupPacket))
...
task<IBuffer^> ControlTransfer::SendVendorControlTransferInToDeviceRecipientAsync(uint8 vendorCommand, uint32 dataPacketLength)
{
    ...
    return create_task(EventHandlerForDevice::Current->Device->SendControlInTransferAsync(initSetupPacket, buffer));
}
