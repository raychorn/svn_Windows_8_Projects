...
void BulkPipes::BulkRead_Click(Object^ /* sender */, RoutedEventArgs^ /* e */)
{
    if (EventHandlerForDevice::Current->IsDeviceConnected)
    {
        ...
        BulkReadAsync(bulkInPipeIndex, bytesToRead).then([this](task<void> readTask)
            {
                try
                {
                    UpdateButtonStates();
...
void BulkPipes::BulkWrite_Click(Object^ /* sender */, RoutedEventArgs^ /* e */)
{
    if (EventHandlerForDevice::Current->IsDeviceConnected)
    {
        ...
        BulkWriteAsync(bulkOutPipeIndex, bytesToWrite).then([this](task<void> writeTask)
            {
                try
                {
                    UpdateButtonStates();
...
void BulkPipes::BulkReadWrite_Click(Object^ /* sender */, RoutedEventArgs^ /* e */)
{
    if (EventHandlerForDevice::Current->IsDeviceConnected)
    {
        ...
        BulkReadWriteAsync(bulkInPipeIndex, bulkOutPipeIndex, bytesToWrite).then([this](task<void> readWriteTask)
            {
                try
                {
                    UpdateButtonStates();
...
{
    if (IsPerformingIo() && !cancellationTokenSource.get_token().is_canceled())
    {
...
task<void> BulkPipes::BulkWriteAsync(uint32 bulkPipeIndex, uint32 bytesToWrite)
{
    ...
    return create_task(writer->StoreAsync(), cancellationTokenSource.get_token())
        .then([this](task<uint32> bytesWrittenTask) 
        {
            // The write operation is no longer running
            runningWriteTask = false;    
...
task<void> BulkPipes::BulkReadAsync(uint32 bulkPipeIndex, uint32 bytesToRead)
{
    ...
    // Cancellation Token will be used so we can stop the task operation explicitly
    // The completion function should still be called so that we can properly handle a canceled task
    return create_task(reader->LoadAsync(bytesToRead), cancellationTokenSource.get_token())
        .then([this, reader] (task<uint32> bytesReadTask) 
        {
            // The read operation is no longer running
            runningReadTask = false;    
...
/// <summary>
/// This method performs pre/post operations of the ReadWrite loop. This makes it so that this method's callers don't have to worry about
/// reseting the state of the loop.
///
/// Any errors in async function will be passed down the task chain and will not be caught here because errors should be 
/// handled at the end of the task chain.
/// </summary>
...
task<void> BulkPipes::BulkReadWriteAsync(uint32 bulkInPipeIndex, uint32 bulkOutPipeIndex, uint32 bytesToReadWrite)
{
    runningReadWriteTask = true;

    return BulkReadWriteLoopAsync(bulkInPipeIndex, bulkOutPipeIndex, bytesToReadWrite).then([this](task<void> readWriteLoopTask) 
    {
        // Post loop
        runningReadWriteTask = false;

        // The reads and writes operation may complete/error after this promise chain. Since we know the isReading and isWriting states
        // will be false, we should propagate the changes to the next .then() callback otherwise the callback will see an incorrect state.
        runningReadTask = false;
        runningWriteTask = false;

        // Pass any errors down
        readWriteLoopTask.get();    
    });
}
...
task<void> BulkPipes::BulkReadWriteLoopAsync(uint32 bulkInPipeIndex, uint32 bulkOutPipeIndex, uint32 bytesToReadWrite)
{
    ...
    {
        task_group readWriteTasks(cancellationTokenSource.get_token());
    
        readWriteTasks.run([this, &readWriteTasks, bulkInPipeIndex, bytesToReadWrite]()
            {
                ...
                {
                    auto readTask = BulkReadAsync(bulkInPipeIndex, bytesToReadWrite);
        ...
        readWriteTasks.run([this, &readWriteTasks, bulkOutPipeIndex, bytesToReadWrite]()
            {
                while (!readWriteTasks.is_canceling() && EventHandlerForDevice::Current->IsDeviceConnected)
                {
                    // While this task is not cancelled, initiate one write after one succeeds
                    auto writeTask = BulkWriteAsync(bulkOutPipeIndex, bytesToReadWrite);
