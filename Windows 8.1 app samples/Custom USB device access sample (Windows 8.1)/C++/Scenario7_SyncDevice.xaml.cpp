...
BackgroundTaskRegistration^ SyncDevice::FindSyncTask()
{
    ...
    while (isBackgroundTask)
    {
        ...
        {
            return dynamic_cast<BackgroundTaskRegistration^>(backgroundTask);
        }
...
task<DeviceTriggerResult> SyncDevice::StartSyncBackgroundTaskAsync(void)
{
    ...
    return create_task(syncBackgroundTaskTrigger->RequestAsync(EventHandlerForDevice::Current->DeviceInformation->Id));
}
...
task<void> SyncDevice::SyncWithDeviceAsync(void)
{
    ...
    return create_task(StartSyncBackgroundTaskAsync()).then([this] (DeviceTriggerResult deviceTriggerResult)
    {
        ...
        if (!isSyncing && errorMessage != nullptr)
        {
            ...
            create_task(EventHandlerForDevice::Current->OpenDeviceAsync(
                EventHandlerForDevice::Current->DeviceInformation,
                EventHandlerForDevice::Current->DeviceSelector)).then([this, errorMessage](task<bool> openDeviceTask)
            {
...
void SyncDevice::OnSyncWithDeviceCompleted(BackgroundTaskRegistration^ /* sender */, BackgroundTaskCompletedEventArgs^ /* args */)
{
    MainPage::Current->Dispatcher->RunAsync(
        ...
        {    
            ...
            create_task(EventHandlerForDevice::Current->OpenDeviceAsync(
                EventHandlerForDevice::Current->DeviceInformation, 
                EventHandlerForDevice::Current->DeviceSelector)).then([this](task<bool> openDeviceTask)
            {
                // Print this status message after the "device was opened" message
                MainPage::Current->Dispatcher->RunAsync(
                    CoreDispatcherPriority::Normal,
                    ref new DispatchedHandler([this]()
                {
                    ...
                    auto taskCompleteStatus = dynamic_cast<String^>(ApplicationData::Current->LocalSettings->Values->Lookup(LocalSettingKeys::SyncBackgroundTaskStatus));

                    if (taskCompleteStatus == SyncBackgroundTaskInformation::TaskCompleted)
                    {
                        String^ totalBytesWritten = dynamic_cast<String^>(ApplicationData::Current->LocalSettings->Values->Lookup(LocalSettingKeys::SyncBackgroundTaskResult));
