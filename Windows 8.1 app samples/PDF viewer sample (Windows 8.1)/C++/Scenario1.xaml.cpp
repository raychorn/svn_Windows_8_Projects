						...
						create_task(tempFolder->CreateFileAsync("test1.png", CreationCollisionOption::ReplaceExisting)).then([this,pdfPage,_pdfDocument,renderOptions](StorageFile^ pngFile)
						{
						  if (pngFile !=nullptr)
						  {
							create_task(pngFile->OpenAsync(FileAccessMode::ReadWrite)).then([this,pdfPage,pngFile,_pdfDocument,renderOptions](IRandomAccessStream^ randomStream)
							{
								...
								create_task(PDFRenderToStreamAsync(pdfPage,randomStream,renderOptions)).then([this,randomStream,pngFile]()
								{
									create_task(randomStream->FlushAsync()).then([this,randomStream,pngFile](bool success)
									{		
										delete randomStream;
										DisplayImageFileAsync(pngFile);
									});															
								});
							 });
						  }
...
{    
	try
    {
        MainPage::Current->NotifyUser("Rendering page...", NotifyType::StatusMessage);
   		RenderPDFPage(PDFFILENAME, RENDEROPTIONS::NORMAL);
 ...
 void PDFAPICPP::Scenario1::DisplayImageFileAsync(StorageFile^ file)
        {          
			create_task(file->OpenAsync(FileAccessMode::Read)).then([this](IRandomAccessStream^ randomStreamNew)
			{
...
void PDFAPICPP::Scenario1::RenderPageZoom_Click(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
	try
    {
        MainPage::Current->NotifyUser("Rendering page at zoom level...", NotifyType::StatusMessage);
   		RenderPDFPage(PDFFILENAME, RENDEROPTIONS::ZOOM);
...
{
	 MainPage::Current->NotifyUser("Rendering portion of a page...", NotifyType::StatusMessage);
     RenderPDFPage(PDFFILENAME, RENDEROPTIONS::PORTION);
