...
(function () {
    "use strict";
    var page = WinJS.UI.Pages.define("/html/S1-LoggingSession.html", {
    ...
    function addLogFileMessage(message, logFilePath) {
        ...
        if (logFilePath !== null && logFilePath.length > 0) {
            ...
            appLogFolder.value = "Sample app log folder: " + getJustDirectoryName(logFilePath);
    ...
    function onStatusChanged(args) {
        ...
        switch (eventData.type) {
            ...
            case "LoggingEnabledDisabled":
                ...
                break;
            case "LogMessageCountUpdate":
                loggingStatistics.textContent =
                    "Total messages logged: " +
                    LoggingScenario.LoggingScenario.loggingScenarioSingleton.getLogMessageCount() +
                    "  Approximate bytes logged: " +
                    LoggingScenario.LoggingScenario.loggingScenarioSingleton.getLogMessageApproximateByteCount();
                break;
        }
    }
    ...
    function enableDisableLogging() {
        WinJS.log && WinJS.log("The scenario is running.", "sample", "status");
        LoggingScenario.LoggingScenario.loggingScenarioSingleton.toggleLoggingEnabledDisabled();
        WinJS.log && WinJS.log("The scenario is finished.", "sample", "status");
        updateControls();
    }
})();
