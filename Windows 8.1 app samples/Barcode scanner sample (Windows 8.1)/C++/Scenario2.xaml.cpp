...
void Scenario2::OnDataReceivedInstance1(Windows::Devices::PointOfService::ClaimedBarcodeScanner ^sender, Windows::Devices::PointOfService::BarcodeScannerDataReceivedEventArgs ^args)
{
		...
		[this,args]()
	{
		...
		DataLabel1->Text = scanDataLabelReader->ReadString(args->Report->ScanDataLabel->Length);

		DataReader^  scanDataReader = DataReader::FromBuffer(args->Report->ScanData);
		ScanData1->Text = scanDataReader->ReadString(args->Report->ScanData->Length);
...
void Scenario2::OnDataReceivedInstance2(Windows::Devices::PointOfService::ClaimedBarcodeScanner ^sender, Windows::Devices::PointOfService::BarcodeScannerDataReceivedEventArgs ^args)
{
	...
	{
		DataReader^ scanDataLabelReader = DataReader::FromBuffer(args->Report->ScanDataLabel);
		DataLabel2->Text = scanDataLabelReader->ReadString(args->Report->ScanDataLabel->Length);

		DataReader^  scanDataReader = DataReader::FromBuffer(args->Report->ScanData);
		ScanData2->Text = scanDataReader->ReadString(args->Report->ScanData->Length);
