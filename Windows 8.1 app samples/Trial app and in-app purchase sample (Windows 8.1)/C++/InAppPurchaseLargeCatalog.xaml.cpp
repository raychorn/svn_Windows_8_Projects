    ...
    create_task(CurrentAppSimulator::RequestProductPurchaseAsync("product1", offerId, displayProperties)).then([this](task<PurchaseResults^> currentTask)
    {
        try
        {
            ...
            case ProductPurchaseStatus::Succeeded:
                ...
                FulfillProduct1("product1", results->TransactionId, results->OfferId);
                break;
...
void InAppPurchaseLargeCatalog::FulfillProduct1(Platform::String ^productId, Platform::Guid transactionId, Platform::String ^offerId)
{
    create_task(CurrentAppSimulator::ReportConsumableFulfillmentAsync(productId, transactionId)).then([this, offerId](task<FulfillmentResult> currentTask)
    {
        Platform::String ^displayPropertiesName = DisplayPropertiesNameTextBox->Text;
        ...
        if (!offerId->IsEmpty())
        {
            offerIdMsg = " with offer id " + offerId;
        }
