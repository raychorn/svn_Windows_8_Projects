...
{
	String^ Connectivity = "Connectivity Level: None";
	...
	if (nullptr != MainPage::Current->connectionSession)
	{
		...
		{
			Connectivity = "Connectivity Level: Connected";
		}
	}

	DisplayInfo(Connectivity);
	return;
}
...
void S1_CreateConnection::Connect_Click(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
	...
	if (fConnecting == true)
	{
		...
		IAsyncOperation<ConnectionSession^>^ op = ConnectivityManager::AcquireConnectionAsync(context);
		...
		acquireConnectionTask.then([this](task<ConnectionSession^> tsession)
		{
			try
			{
				...
				DisplayInfo("Succesfully acquired the requested connection");
			}
			catch(Exception^ e)
			{
	...
	{	// DISCONNECTING
		if (MainPage::Current->connectionSession != nullptr)
		{
			...
			MainPage::Current->connectionSession = nullptr;
			DisplayInfo("Succesfully disconnected the requested connection");
		}
	}
}
