...
(function () {
    ...
    var page = WinJS.UI.Pages.define("/html/scenario4DeviceAutoConfiguredScan.html", {
    ...
    function startScenario() {
        WinJS.log && WinJS.log("Starting scenario of Device Auto-Configured Scanning.", "sample", "status");
        resetDisplay();
        ...
        Windows.Devices.Scanners.ImageScanner.fromIdAsync(deviceId).then(function (myScanner) {
            ...
            if (scenarioRunning) {
                if (myScanner.isScanSourceSupported(Windows.Devices.Scanners.ImageScannerScanSource.autoConfigured)) {
                    ...
                    if (myScanner.autoConfiguration.isFormatSupported(Windows.Devices.Scanners.ImageScannerFormat.png)) {
                        myScanner.autoConfiguration.format = Windows.Devices.Scanners.ImageScannerFormat.png;
                    }
            ...
            if (result) {
                if (result.scannedFiles.size > 0) {
                    ...
                    WinJS.log && WinJS.log("There were no files scanned." + scannedFileName, "sample", "error");
                }
            }
        }, function (error) {
            if (error.name === "Canceled") {
                Utils.dispalyScanCancelationMessage();
            } else {
                resetDisplay();
                Utils.dispalyErrorMessage(error);
            }
        },
        function (progress) {
