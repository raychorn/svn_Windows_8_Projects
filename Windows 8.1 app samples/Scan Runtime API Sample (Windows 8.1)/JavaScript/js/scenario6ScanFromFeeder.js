...
(function () {
    ...
    var page = WinJS.UI.Pages.define("/html/scenario6ScanFromFeeder.html", {
    ...
    /// <summary>
    /// Event Handler for click on start scenario button. Starts the scenario for scanning from the feeder.
    /// </summary>
    function startScenario() {
        WinJS.log && WinJS.log("Starting scenario of scanning from Feeder.", "sample", "status");
        resetDisplay();
        ...
        Windows.Devices.Scanners.ImageScanner.fromIdAsync(deviceId).then(function (myScanner) {
            ...
            if (result) {
                if (result.scannedFiles.size > 0) {
                    ...
                    WinJS.log && WinJS.log("There are no files scanned from the Feeder." + scannedFileName, "sample", "error");
                }
            }
        }, function (error) {
