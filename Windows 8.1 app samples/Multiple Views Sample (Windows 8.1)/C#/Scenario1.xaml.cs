...
namespace MultipleViews
{
    ...
    public sealed partial class Scenario1 : SDKTemplate.Common.LayoutAwarePage
    {
        ...
        private async void CreateView_Click(object sender, RoutedEventArgs e)
        {
            ...
            // The actual creation of the view is done in OnWindowCreate in App.xaml.cs
            CoreDispatcher newDispatcher = await ((App)App.Current).CreateNewView();

            ViewData viewData = null;
            await newDispatcher.RunAsync(CoreDispatcherPriority.Normal, () =>
            {
                ...
                viewData.Title = DEFAULT_TITLE;

                var frame = new Frame();
                frame.Navigate(typeof(SecondaryViewPage), viewData);
                Window.Current.Content = frame;

                ApplicationView.GetForCurrentView().Title = viewData.Title;
            });

            // Be careful! This collection is bound to the current thread,
            // so make sure to update it only from this thread
            ((App)App.Current).SecondaryViews.Add(viewData);
        }

        private async void ShowAsStandalone_Click(object sender, RoutedEventArgs e)
        {
            var selectedView = ViewChooser.SelectedItem as ViewData;
