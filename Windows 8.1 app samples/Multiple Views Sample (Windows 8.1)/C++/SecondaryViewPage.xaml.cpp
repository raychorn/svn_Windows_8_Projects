...
{
    thisViewData = dynamic_cast<ViewData^>(e->Parameter);
    ...
    releasedToken = thisViewData->Released += ref new ViewDataReleasedHandler(this, &SecondaryViewPage::viewData_Released);
}
...
void SecondaryViewPage::SetTitle(String^ newTitle)
{
    ...
    // The title contained in the ViewData object is bound to
...
void SecondaryViewPage::viewData_Released(Object^ sender, Object^ e)
{
    dynamic_cast<ViewData^>(sender)->Released -= releasedToken;
    
    auto thisDispatcher = Window::Current->Dispatcher;

    // The ViewData object is bound to UI elements on the main thread
        ...
        ref new DispatchedHandler([this, thisDispatcher] ()
    {
        ...
        for (unsigned int i = 0; i < views->Size; i++)
        {
            ...
            if (thisViewData->Id == viewData->Id)
            {
                views->RemoveAt(i);
