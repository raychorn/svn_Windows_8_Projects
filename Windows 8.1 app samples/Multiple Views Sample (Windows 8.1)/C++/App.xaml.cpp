...
App::App()
{
    ...
    secondaryViews = ref new Vector<ViewData^>();
}

IVector<ViewData^>^ App::SecondaryViews::get()
{
...
void App::OnWindowCreated(WindowCreatedEventArgs^ pArgs)
{
    ...
    if (!view->IsHosted && !view->IsMain)
    {
        std::shared_ptr<task_completion_event<CoreDispatcher^>> creationTask;
        if (viewCreationTasks.try_pop(creationTask))
        {
            creationTask->set(pArgs->Window->Dispatcher);
        }
        else
        {
...
void App::OnLaunched(LaunchActivatedEventArgs^ args)
{
    ...
    ViewData^ viewData;
    if (TryFindViewDataForViewId(args->CurrentlyShownApplicationViewId, &viewData))
    {
...
void App::OnActivated(IActivatedEventArgs^ pArgs)
{
    if (pArgs->Kind == ActivationKind::Protocol)
    {
        ...
        ViewData^ viewData;
        if (TryFindViewDataForViewId(protocolArgs->CurrentlyShownApplicationViewId, &viewData))
        {
            create_task(viewData->Dispatcher->RunAsync(CoreDispatcherPriority::Normal,
...
bool App::TryFindViewDataForViewId(int viewId, SDKSample::MultipleViews::ViewData^* foundData)
{
...
{
    ViewData^ viewData;
    bool found = TryFindViewDataForViewId(viewId, &viewData);
    if (found)
    {
        viewData->Title = newTitle;
    }
    else
    {
...
task<CoreDispatcher^> App::CreateNewView()
{
    auto pTce = std::make_shared<task_completion_event<CoreDispatcher^>>();
    viewCreationTasks.push(pTce);

    CoreApplication::CreateNewView("", "");

    return task<CoreDispatcher^>(*pTce);
}