...
void Scenario1::CreateView_Click(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
    ...
    auto viewData = std::make_shared<ViewData^>();
    
    dynamic_cast<App^>(App::Current)->CreateNewView().then([viewData] (CoreDispatcher^ newDispatcher)
    {
        auto thisDispatcher = Window::Current->Dispatcher;
        return newDispatcher->RunAsync(CoreDispatcherPriority::Normal, ref new DispatchedHandler([viewData, thisDispatcher] ()
        {
            ...
            auto newViewData = ViewData::CreateForCurrentView();
            *viewData = newViewData;

            newViewData->Title = ref new String(DEFAULT_TITLE);

            auto frame = ref new Windows::UI::Xaml::Controls::Frame();
            frame->Navigate(TypeName(SecondaryViewPage::typeid), newViewData);
            Window::Current->Content = frame;

            ApplicationView::GetForCurrentView()->Title = newViewData->Title;

            // Be careful! This collection is bound to the main thread,
            // so make sure to update it only from this thread
            thisDispatcher->RunAsync(CoreDispatcherPriority::Normal, ref new DispatchedHandler([viewData] ()
            {
                dynamic_cast<App^>(App::Current)->SecondaryViews->Append(*viewData);
            }));
        }));
...
{
    auto selectedView = dynamic_cast<ViewData^>(ViewChooser->SelectedItem);
