...
DeviceList^ DeviceList::current = nullptr;

DeviceList^ DeviceList::Current::get()
{
    if (DeviceList::current == nullptr) {
        DeviceList::current = ref new DeviceList();
    }
    return DeviceList::current;
}
...
{
    String^ selector = L"System.Devices.InterfaceClassGuid:=\"{DEE824EF-729B-4A0E-9C14-B7117D33A817}\" AND System.Devices.InterfaceEnabled:=System.StructuredQueryType.Boolean#True"; // all UsbDevice

    // Create a device watcher to look for instances of the  device interface
    this->watcher = DeviceInformation::CreateWatcher(selector);
