...
task<void> FirmwareUpdate::UpdateFirmwareForFirstEnumeratedDeviceAsync()
{
    return FindFirstSuperMuttDeviceAsync().then([this] (DeviceInformation^ firstSuperMuttDeviceFound)
        {
            deviceToDoFirmwareUpdate = firstSuperMuttDeviceFound;
        }).then([this] ()
        {
            if (deviceToDoFirmwareUpdate != nullptr)
            {
                // Open device here and get permission from the user
                return create_task(UsbDevice::FromIdAsync(deviceToDoFirmwareUpdate->Id)).then([this] (UsbDevice^ usbDevice)
                    {
                        if (usbDevice != nullptr)
                        {
                            ...
                            return StartFirmwareForDeviceAsync(deviceToDoFirmwareUpdate);
                        }
                        else
                        {
                            return task<String^>([] ()
    ...
    Windows::ApplicationModel::Background::BackgroundTaskCompletedEventArgs^ /* args */)
{
    MainPage::Current->Dispatcher->RunAsync(
        ...
        {    
            ...
            auto taskCompleteStatus = dynamic_cast<String^>(ApplicationData::Current->LocalSettings->Values->Lookup(LocalSettingKeys::FirmwareUpdateBackgroundTask::TaskStatus));

            if (taskCompleteStatus == FirmwareUpdateTaskInformation::TaskCompleted)
            {
                // Display firmware version after the firmware update
                create_task(UsbDevice::FromIdAsync(deviceToDoFirmwareUpdate->Id)).then([this] (UsbDevice^ usbDevice)
                {
                    auto newFirmwareVersion = Utilities::ConvertToHex16Bit(usbDevice->DeviceDescriptor->BcdDeviceRevision);
                    UpdateNewFirmwareVersionInUI(newFirmwareVersion);

                    // Close the device because we no longer need it
                    delete usbDevice;
                    usbDevice = nullptr;
                    deviceToDoFirmwareUpdate = nullptr;
                });
