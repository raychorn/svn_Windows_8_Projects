...
pdfLibrary = {
    ...
    loadPages: function (startIndex, endIndex, pdfDocument, pdfPageRenderingOptions, inMemoryFlag, tempFolder) {
        ...
        for (var count = startIndex; count < endIndex; count++) {
            var promise = pdfLibrary.loadPage(count, pdfDocument, pdfPageRenderingOptions, inMemoryFlag, tempFolder).then(function (pageData) {
                return pageData;
            });
            ...
            .then(function closeStream() {
                ...
                if (inMemoryFlag) {
                    ...
                    return renderStream.openReadAsync().then(function (stream) {
                        ...
                        // Object URL of the rendered page in memory
                        picURL = URL.createObjectURL(stream, { oneTimeOnly: false });
                    ...
                    if (filePointer === null) {
                        picURL = "images\\placeholder.jpg";
                    } else {
                        // Object URL of the rendered page on disk
                        picURL = URL.createObjectURL(filePointer, { oneTimeOnly: false });
                    }
                    return { pageIndex: pageIndex, imageSrc: picURL };
                }
            });
