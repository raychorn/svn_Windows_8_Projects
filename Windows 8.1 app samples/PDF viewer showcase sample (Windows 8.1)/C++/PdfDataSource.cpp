...
namespace PdfShowcase
{
	...
	Concurrency::task<Windows::UI::Xaml::Data::LoadMoreItemsResult> PdfDataSource::LoadMoreItemsOverride(_In_ Concurrency::cancellation_token c, _In_ unsigned int count)
	{
	...
	Platform::Object^ PdfDataSource::LoadPages(_In_ int index, _In_ PdfDocument^ pdfDocument)
	{
		...
		case SurfaceType::SurfaceImageSource:
			...
			// Dispatching task on UI thread to render PDF page image on SIS surface
			run_async_non_interactive(Windows::UI::Core::CoreDispatcherPriority::Low, [this, imageSource](void)
			{
