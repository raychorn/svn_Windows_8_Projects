...
namespace PdfShowcase
{
	...
	Platform::Object^ IncrementalLoadingBase::default::get(_In_ int index)
	{
		return storage->GetAt(index);
	}
	void IncrementalLoadingBase::default::set(_In_ int index, _In_ Platform::Object^ value)
	{
	...
	Concurrency::task<LoadMoreItemsResult> IncrementalLoadingBase::LoadMoreItemsAsync(_In_ Concurrency::cancellation_token c, _In_ unsigned int count)
	{
		// Creating a concurrent task to involve override method to load the pages
	...
	Concurrency::task<LoadMoreItemsResult> IncrementalLoadingBase::LoadMoreItemsOverride(_In_ Concurrency::cancellation_token c, _In_ unsigned int count)
	{
		return Concurrency::task<LoadMoreItemsResult>(
			[]()->LoadMoreItemsResult {
	...
	IncrementalLoadingBase::IncrementalLoadingBase()
	{
		...
		storage = ref new Platform::Collections::Vector<Platform::Object^>();
		storage->VectorChanged += ref new Windows::Foundation::Collections::VectorChangedEventHandler<Platform::Object^>(this, &IncrementalLoadingBase::StorageVectorChanged);
	...
	Windows::Foundation::IAsyncOperation<LoadMoreItemsResult>^ IncrementalLoadingBase::LoadMoreItemsAsync(_In_ unsigned int count)
	{
		// Creating cancelation token which will be used for canceling tasks if required
		auto token = cts.get_token();

		// Creating async task for loading items
		return Concurrency::create_async([this, count](Concurrency::cancellation_token token) {
	...
	void IncrementalLoadingBase::VectorChanged::raise(_In_ Windows::UI::Xaml::Interop::IBindableObservableVector^ vector, _In_ Platform::Object^ e)
	{
	...
	void IncrementalLoadingBase::Append(_In_ Platform::Object^ value)
	{
	...
	Platform::Object^ IncrementalLoadingBase::GetAt(_In_ unsigned int index)
	{
		return storage->GetAt(index);
	}
	...
	bool IncrementalLoadingBase::IndexOf(_In_ Platform::Object^ value, _In_ unsigned int* index)
	{
		return storage->IndexOf(value, index);
	}

	void IncrementalLoadingBase::InsertAt(_In_ unsigned int index, _In_ Platform::Object^ value)
	{
	...
	void IncrementalLoadingBase::SetAt(_In_ unsigned int index, _In_ Platform::Object^ value)
	{
		storage->SetAt(index, value);
	}
	...
	void IncrementalLoadingBase::StorageVectorChanged(_In_ Windows::Foundation::Collections::IObservableVector<Platform::Object^>^ /*sender*/, _In_ Windows::Foundation::Collections::IVectorChangedEventArgs^ e)
	{
		if (isVectorChangedObserved)
		{
