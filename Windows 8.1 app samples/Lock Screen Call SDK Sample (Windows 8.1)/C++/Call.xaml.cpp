...
void LockScreenCall::CallPage::OnNavigatedTo(Windows::UI::Xaml::Navigation::NavigationEventArgs^ e)
{
    ...
    if (lockScreenArgs != nullptr)
    {
        ...
        closedToken = callUI->Closed += ref new TypedEventHandler<LockScreenCallUI^, Platform::Object^>(this, &CallPage::OnClosed);

        // Set the title.
        callUI->CallTitle = ref new Platform::String(callTitle.c_str());
    }

    CallImage->Text = ref new Platform::String(imageText.c_str());
    
    // Assign a random light background color so that each call looks
    // slightly different.
    LayoutRoot->Background = RandomLightSolidColor();
            
    CallTitle->Text = ref new Platform::String(callTitle.c_str());
...
void LockScreenCall::CallPage::OnNavigatedFrom(Windows::UI::Xaml::Navigation::NavigationEventArgs^ e)
{
    if (callUI != nullptr)
    {
...
void LockScreenCall::CallPage::NavigateToMainPage()
{
    Frame->Navigate(TypeName(MainPage::typeid), nullptr);
}

void LockScreenCall::CallPage::QueueOnUIThread(DispatchedHandler^ agileCallback)
{
    Dispatcher->RunAsync(Windows::UI::Core::CoreDispatcherPriority::Normal, agileCallback);
}

// Fade to black then invokes the provided handler.
void LockScreenCall::CallPage::FadeToBlackThen(EventHandler<Platform::Object^>^ agileCallback)
{
    CallFadeOut->Visibility = Windows::UI::Xaml::Visibility::Visible;
    auto eventToken = new EventRegistrationToken;
    auto completed = ref new EventHandler<Platform::Object^>([=](Platform::Object^ sender, Platform::Object^ o)
    {
...
void LockScreenCall::CallPage::OtherPartyHangsUp(Windows::System::Threading::ThreadPoolTimer^ sender)
{
    QueueOnUIThread(ref new Windows::UI::Core::DispatchedHandler([=]()
    {
        if (callUI != nullptr)
        {
            FadeToBlackThen(ref new EventHandler<Platform::Object^>([=](Platform::Object^ sender, Platform::Object^ o)
            {
...
void LockScreenCall::CallPage::OnEndRequested(LockScreenCallUI^ sender, LockScreenCallEndRequestedEventArgs^ e)
{
...
void LockScreenCall::CallPage::OnClosed(LockScreenCallUI^ sender, Platform::Object^ e)
{
    QueueOnUIThread(ref new Windows::UI::Core::DispatchedHandler([=]()
    {
        callUI = nullptr;
...
void LockScreenCall::CallPage::EndCallButton_Click(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
    NavigateToMainPage();
}
