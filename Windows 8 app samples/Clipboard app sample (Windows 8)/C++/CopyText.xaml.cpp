        ...
        create_task(dataPackageView->GetHtmlFormatAsync()).then(
            [this](task<String^> htmlTask)
            {
                ...
                if (htmlFormat != nullptr)
                {
                    ...
                    // Note: Metro Style apps may be unable to render some schemes (e.g., 'ms-clipboard-file') that reference local content.
                    // As a result, images may look broken. To learn the different URL schemes used by Metro style apps and how to use them
