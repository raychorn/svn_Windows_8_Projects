...
Namespace Global.SDKTemplate
    ...
    Partial Public NotInheritable Class MainPage
        ...
        Public Sub New()
            ...
            CreateClient(New Uri(BaseAddress.Text))
        End Sub

        Private Sub CreateClient(server As Uri)
            If httpClient IsNot Nothing Then
                httpClient.Dispose()
            End If
            Dim handler As HttpMessageHandler = New HttpClientHandler()
        ...
        Friend Function TryUpdateBaseAddress() As Boolean
            ' Update the target server
            Dim server As Uri = Nothing
            If Not Uri.TryCreate(BaseAddress.Text, UriKind.Absolute, server) Then
                BaseAddress.Text = "http://localhost/HttpClientSample/"
                Return False
            End If

            If Not server.Equals(httpClient.BaseAddress) Then
                CreateClient(server)
            End If
        ...
        Private Sub InvalidateViewState()
            ...
            If ApplicationView.Value = ApplicationViewState.Snapped Then
                Grid.SetRow(BaseAddress, 2)
                Grid.SetColumn(BaseAddress, 0)

                Grid.SetRow(DescriptionText, 5)
                Grid.SetColumn(DescriptionText, 0)

                Grid.SetRow(InputSection, 6)
                Grid.SetColumn(InputSection, 0)

                Grid.SetRow(FooterPanel, 2)
                Grid.SetColumn(FooterPanel, 0)
            Else
                Grid.SetRow(BaseAddress, 1)
                Grid.SetColumn(BaseAddress, 1)
