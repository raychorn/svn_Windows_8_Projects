...
MainPage::MainPage()
{
	...
	BottomAppBar->Loaded += ref new RoutedEventHandler(this, &MainPage::BottonAppBar_Loaded);
...
void MainPage::BottonAppBar_Loaded(Object^ sender, RoutedEventArgs^ e)
{
	RegisterAppBarButtons();
}

void MainPage::RegisterAppBarButtons()
{
	Grid^ rootGrid = safe_cast<Grid^>(BottomAppBar->Content);
	for each (UIElement^ e in rootGrid->Children)
	{
		for each (UIElement^ c in ((StackPanel^)e)->Children)
		{
			if (c->GetType()->FullName == Button::typeid->FullName)
			{
				AppBarControl::Common::LayoutAwarePage::StartLayoutUpdates(c, nullptr);
			}
		}
	}
}
...
void MainPage::LoadScenario(String^ scenarioName)
{
    ...
    // LoTypeName scenarioType = {scenarioName, TypeKind::Custom};ad the ScenarioX.xaml file into the Frame.
