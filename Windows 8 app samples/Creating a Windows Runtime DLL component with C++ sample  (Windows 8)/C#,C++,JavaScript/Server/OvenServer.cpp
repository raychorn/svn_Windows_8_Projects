...
namespace ABI { namespace Microsoft { namespace SDKSamples { namespace Kitchen {
    ...
    {
        *ppOven = nullptr;
        ComPtr<Oven> spOven = Make<Oven>();
        HRESULT hr = (spOven == nullptr ? E_OUTOFMEMORY : S_OK);
        if (SUCCEEDED(hr))
        {
    ...
    {
        *ppOven = Make<Oven>(dimensions).Detach();
        return (*ppOven == nullptr ? E_OUTOFMEMORY : S_OK);
    }
                ...
                {    
                    ...
                    auto spTimerCallback = Callback<::ABI::Windows::System::Threading::ITimerElapsedHandler>(
                        [spThis, hstrFlavorCopy](::ABI::Windows::System::Threading::IThreadPoolTimer *) -> HRESULT
                        {
                            ...
                            if (SUCCEEDED(hr))
                            {
                                spThis->_evtBreadComplete.InvokeAll(spThis.Get(), spBread.Get());
                            }
