...
Partial Public NotInheritable Class Scenario10
    ...
    Private Sub Scenario10Button_Show_Click(sender As Object, e As RoutedEventArgs)
        ...
        If b IsNot Nothing Then
            Dim context = New ResourceContext()
    ...
    Private Sub Scenario10_SearchMultipleResourceIds(context As ResourceContext, resourceIds As String())
    ...
    Private Sub Scenario10_ShowCandidates(resourceId As String, resourceCandidates As IReadOnlyList(Of ResourceCandidate))
        ' print 'resourceId', 'found value', 'qualifer info', 'matching condition'
        Dim outText As String = "resourceId: dimensions\" & resourceId + vbCr & vbLf
        Dim i As Integer = 0
        For Each candidate In resourceCandidates
            Dim value = candidate.ValueAsString
            outText += "    Candidate " & i.ToString & ":" & value + vbCr & vbLf
            Dim j As Integer = 0
            For Each qualifier In candidate.Qualifiers
                ...
                outText += "        Qualifer: " & qualifierName & ": " & qualifierValue + vbCr & vbLf
                outText += "        Matching: IsMatch (" & qualifier.IsMatch.ToString & ")  " & "IsDefault (" & qualifier.IsDefault.ToString & ")" & vbCr & vbLf
                j += 1
            Next
            i += 1
        Next

        Me.Scenario10TextBlock.Text += outText + vbCr & vbLf
