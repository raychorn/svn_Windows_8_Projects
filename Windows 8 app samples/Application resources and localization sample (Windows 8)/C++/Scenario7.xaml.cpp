...
void Scenario7::OnNavigatedFrom(NavigationEventArgs^ e)
{
    ...
    {
        ResourceManager::Current->DefaultContext->QualifierValues->MapChanged -= eventToken;
        eventToken.Value = 0;
    }
} 
...
void Scenario7::Scenario7Button_Show_Click(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
    if (eventToken.Value == 0)
    {
        Scenario7TextBlock->Text = ResourceManager::Current->MainResourceMap->GetValue("Resources/string1")->ValueAsString;
        eventToken = ResourceManager::Current->DefaultContext->QualifierValues->MapChanged += ref new MapChangedEventHandler<String^, String^>(this, &Scenario7::ContextChanged);
    }
}
