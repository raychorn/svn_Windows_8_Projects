...
void Scenario2::ConnectSocket_Click(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
    ...
    socket->MessageReceived += ref new TypedEventHandler<DatagramSocket^, DatagramSocketMessageReceivedEventArgs^>(socketContext, &SocketContext::OnMessage);

    // Events cannot be hooked up directly to the ScenarioInput2 object, as the object can fall out-of-scope and be deleted. This would render
    // any event hooked up to the object ineffective. The SocketContext guarantees that both the socket and object that serves it's events have the same life time.
    ...
    task<void>(socket->ConnectAsync(hostName, ServiceNameForConnect->Text)).then([this, socketContext] (task<void> previousTask)
    {
        try
        {
...
SocketContext::~SocketContext()
{
    ...
    //  - explicit: by using delete operator (the socket or the stream used by data writer is closed even if there are outstanding references to the objects).
...
void SocketContext::OnMessage(DatagramSocket^ socket, DatagramSocketMessageReceivedEventArgs^ eventArguments)
{
    ...
    catch (Exception^ exception)
    {
        ...
        if (socketError == SocketErrorStatus::ConnectionResetByPeer)
        {
            ...
            NotifyUserFromAsyncThread("Peer does not listen on the specific port. Please make sure that you run step 1 first " +
                "or you have a server properly working on a remote server.", NotifyType::ErrorMessage);
        }
        else if (socketError != SocketErrorStatus::Unknown)
        {
            NotifyUserFromAsyncThread("Error happened when receiving a datagram: " + socketError.ToString(), NotifyType::ErrorMessage);
        }
        else
        {
            throw;
