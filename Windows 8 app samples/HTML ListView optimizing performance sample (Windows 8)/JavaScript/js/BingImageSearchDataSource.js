...
(function () {
    //This should not be included or used in real apps, it's here to illustrate differences between renderers
    function delayPromise(p, delay) {
        var tm = new WinJS.Promise(function (complete, error, progress) {
            var timeout = setTimeout(function () { complete(); }, delay);
        });
        return WinJS.Promise.join([p, tm]).then(function () { return p; });
    }
    ...
    var bingImageSearchDataAdapter = WinJS.Class.define(
        ...
        // These methods will be called by virtualized datasource to fetch items, count etc.
        {
            ...
            getCount: function () {
                ...
                return delayPromise(WinJS.xhr({ url: requestStr, user: "foo", password: that._devkey, }), that._delay).then(
